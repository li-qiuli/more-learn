...
module.exports = {
  ...
  optimization: {
    ...
    splitChunks: {
      ...
      // maxAsyncRequests: 30, // 按需加载时并行加载的文件的最大数量（数量过大，对服务器压力过大）
...
1. 统一命名配置[开发、生产](chunkFilename)【css（插件）、图片与字体样式（output）】
输出文件的命名
有哪些命名：
* output的filename
* chunkFilenamed的动态加载文件（或node_modules）
* 图片、字体图标
* 打包样式
...
module.exports = {
  ...
  output: {
    ...
    // 图片、字体等通过type:asset处理的资源命名方式（注意用hash）
    clean: true,
  },
  module: {
    rules: [
...
## Network Cache(runtime文件缓存配置)

1.资源更新，自动请求更新的资源（contenthash）
2.资源发生变化，只变化变化的资源，不变继续使用缓存（runtimeChunk）

 optimization.runtimeChunk: {
   name:()=>
 }
### 为什么
一个文件改变，main依赖改变的文件，main也会重新构建。开发时我们对静态资源会使用缓存来优化，这样浏览器第二次请求资源就能读取缓存了，速度很快。
...
根据文件内容生成 hash 值，只有**文件内容变化**了，hash 值才会变化。所有文件 hash 值是独享且不同的。
...
## Core-js(es兼容性问题[配置babel:按需引入--例如：arr.push，自动解析])
