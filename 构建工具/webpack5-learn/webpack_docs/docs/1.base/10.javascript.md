...
原因是 Webpack 对 js 处理是有限的，只能编译 js 中 ES 模块化语法，不能编译其他语法(箭头函数，三点运算符)，导致 js 不能在 IE 等浏览器运行，所以我们希望做一些**兼容性处理**。
...
3. extends 继承(官方的规则不用下载，其他规则要下载)
...
1. presets 预设（官方插件或预设的插件）

简单理解：就是一组 Babel 插件, 扩展 Babel 功能

- `@babel/preset-env`: 一个智能预设，允许您使用最新的 JavaScript。【箭头函数，三目】==>es5
- `@babel/preset-react`：一个用来编译 React jsx 语法的预设==>js
- `@babel/preset-typescript`：一个用来编译 TypeScript 语法的预设==>js

### 3. 在 Webpack 中使用babel
