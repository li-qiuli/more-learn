...
默认情况下，资源文件会被转化为 UTF-8 字符串，然后传给 loader。通过设置 raw 为 true，loader 可以接收原始的 Buffer（二进制数据：图片，字体图标）。
...
pitch方法会在loader执行之前执行(并且pitch方法，从左到右)，加return 会中断带pitch方法loader的执行，
```js
module.exports = function (content) {
  return content;
};
...
```json
{
  ...
  // 是否允许追加属性：false不能追加属性-options只能有“author”字段，添加其他字段（或类型不一致）会报错
