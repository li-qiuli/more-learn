...
* 之间在render里写行内的箭头函数（不推荐） 
* 在组件内使用箭头函数定义一个方法（推荐） 
* 直接在组件内定义一个非箭头函数的方法，然后在render里直接私用onClick={this.handleClick.bind(this)}  (不推荐) 
* 在组件内使用非箭头函数，然后在constructor里bind(this) (推荐) 

匿名函数改变this指向:this.handle.bind(this) 
JS 改变this指向：call(this指向后自动执行函数)、apply(this指向后自动执行函数)、bind 
```javascript
var obj1={ 
...
react不会真正的绑定事件到具体某一个节点，而是采用事件代理模式绑定到根结点，通过事件冒泡实现 

 

优点：节点绑定事件消耗内存，此方法可节省内存；不用事件解绑 
...
this.setState //处在异步中，同步更新状态、真实dom 
...
react标识位，点完按钮的同步事件中设置为true，表示setState可以合并处理； 

异步状态，react标识位为false,不支持合并，不阻塞线程执行。 

 

怎样知道状态、dom是否更新完？setState第二个参数回调函数，状态合dom更新完后会被触发 this.setState（{},()=>{console.log(this.state)}） 
...
2. ref标记（父组件拿到子组件的引用，从而调用子组件的方法）---登陆界面 
...
react-----props.children----(数组：可加[0] [1] 等  插槽标记) 

用处：1.为了复用（轮播【图片、文字】、选项卡）；2.一定程度减少父子通信 
...
1. 初始化 
- compontentWillMount :          render之前最后一次修改状态的机会 
===>UNDAFE_compontentWillMount （renamed，不建议使用） 
- Render:只能访问this.props和this.state,不允许修改状态和Dom输出 
- compontentDidMount :          成功render并渲染完成真实DOM之后触发，可以修改DOM（数据请求、订阅函数调用、setInterval、基于创建完成的dom,初始化。。。） 
2. 运行中 

- compontentWillReceiveProps：父组件修改属性触发 
- shouldCompontentUpdate: 返回false会阻止render调用 
- compontentWillUpdate :不能修改属性和状态 
- render :只能访问this.props和this.state,不允许修改状态和Dom输出 
- compontentDidUpdate:成功render并渲染完成真实DOM之后触发，可以修改DOM 

3. 销毁阶段 

- compontentWillUnmount:删除组件之前进行清理工作，比如：计时器和事件监听器 
...
## 问题： 
react 创建state会创建虚拟Dom树，对比老的虚拟Dom树，是同步的；数据量过多时，造成浏览器假死 
Fiber找出要更新的dom（优先级）可打断；更新dom不可打断 
WillMount 找出要更新 低优先级、可打断 
render 开始更新，不可打断 
...
帮你比较新、旧props/state 根据return true/false 决定要不要render 
...
2. useEffect(处理副作用)和useLayoutEffect(同步执行副作用) ,function组件不存在生命周期 
    -  useEffect(()=>{ 接口请求 },[])    //传空数组：不依赖任何东西，只执行一次 

    - //除了第一次执行，之后传依赖根据也会根据依赖更新 
